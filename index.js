...
async function main() {
  ...
  // 3) 2025년 K리그1 FC서울 / 울산 스쿼드 조회 (🔥 여기서부터 await 필요)
  const seoulPlayers2025 = await getPlayersByTeamInSeason(
    2025,
    "K1",
    TEAM_IDS.SEOUL
  );
  const ulsanPlayers2025 = await getPlayersByTeamInSeason(
    2025,
    "K1",
    TEAM_IDS.ULSAN
  );

  console.log("\n👥 2025년 K리그1 FC서울 스쿼드 (DB 기반):");
  seoulPlayers2025.forEach((player, index) => {
    console.log(
      `${index + 1}. ${player.name} | 포지션: ${player.position} | 생년월일: ${player.birth_date} | 등번호: ${player.squad_number}`
    );
  });

  console.log("\n👥 2025년 K리그1 울산 HD 스쿼드 (DB 기반):");
  ...
  // 7) DB에서 팀 리스트 불러오기 (기존 테스트)
  const teamsFromDB = await getAllTeamsFromDB();
  console.log("\n📡 DB에서 읽은 팀 목록:");
...
// 🔥 메인 실행
main().catch((err) => {
  console.error("❌ index.js 실행 중 오류:", err);
});
